...
export function MobiliadoQualifyingForm() {
  const [showPoll, setShowPoll] = useState(false);
  const [selectedOption, setSelectedOption] = useState<string | null>(null);

  const whatsappUrl = (entrada: string) => {
    const mensagem = `Olá Cátia! Tenho interesse no Apartamento Mobiliado Baependi (R$ 348.000). Sobre a entrada: ${entrada}. Gostaria de mais informações e agendar uma visita.`;
